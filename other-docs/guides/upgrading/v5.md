...
The MSM Sitemaps plugin used to provide the XML Sitemaps feature however this has been superceded by the new core Sitemaps in WordPress 5.5. These were a collaboration between WordPress, Google and Human Made.

The update may require you to re-submit your sitemap index file in [Google Search Console](https://search.google.com/search-console/) as it has been relocated from `/sitemap.xml` to `/wp-sitemap.xml`. The new index sitemap URL is automatically added to the `robots.txt` file.

The filters provided by MSM Sitemaps that were previously documented are no longer available. Please [refer to updated Sitemaps documentation](docs://seo/xml-sitemaps.md) to learn how to achieve the same results.
