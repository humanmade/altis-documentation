...
Intermediate image size files are no longer created but handled entirely by Tachyon if `modules.media.smart-media` is enabled in your config, this is on by default. This is only a breaking change if you have code that relies on fetching those files (not the original or full size image) directly by their path.

### Block Editor

The updated Block Editor in WordPress 5.3 has many updates to the underlying javascript packages such as React itself and features of the `@wordpress/data` package. If you make use of helpers like `wp.data.withDispatch` you should test this update thoroughly.
...
- Large image files are stored separately as the original and a web optimised full size version is generated for use on the website to improve performance
...
Tests are run on the local environment whether it is [Local Server](docs://local-server) or [Local Chassis](docs://local-chassis) and as such provide full integration testing capabilities to match production with S3, ElasticSearch, caching and Tachyon.
...
- Automatic Sub Resource Integrity hashes for 1st party scripts with 3rd party support via filters
