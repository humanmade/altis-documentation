...
Each standard has one or more corresponding sniff codes, which are also shown in the ACR ([Automated Code Review](README.md#automated-code-review)) output. These sniffs can be used to [ignore the error](README.md#ignore) if they are false positives, but note that this will then require manual code review by the Altis Cloud team.
